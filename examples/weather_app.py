...
from lmae.actor import Line, SpriteImage, StillImage, Text
...
class WeatherApp(DisplayManagedApp):
    ...
    def __init__(
        ...
        self.logger.info(
            f"Weather conditions for location at lat/long {self.latitude}, {self.longitude}"
        )
        ...
        self.cloudy_image = Image.open(
            os.path.join(resource_path, "images/backgrounds/cloudy.png")
        ).convert("RGBA")
        self.dark_clouds_image = Image.open(
            os.path.join(resource_path, "images/backgrounds/dark_clouds.png")
        ).convert("RGBA")
        self.night_sky_image = Image.open(
            os.path.join(resource_path, "images/backgrounds/night_sky.png")
        ).convert("RGBA")
        self.sunrise_sunset_image = Image.open(
            os.path.join(resource_path, "images/backgrounds/sunrise_sunset.png")
        ).convert("RGBA")
        ...
        self.temperature_label = Text(
            name="TemperatureActor",
            position=(5, 7),
            font=self.primary_text_font,
        ...
        # feels like actor
        self.feels_like_label = Text(
            name="FeelsLikeActor",
            position=(5, 24),
            font=self.secondary_text_font,
        ...
        # carousel for temps
        self.temps_carousel = Carousel(
            name="TempsCarousel",
            crop_area=(4, 23, 38, 31),
        ...
        self.main_daytime_image = SpriteImage(
            name="main-daytime-condition",
            position=(39, 10),
            sheet=sprite_sheet,
            spec=sprite_spec,
        )
        self.support_daytime_image_1 = SpriteImage(
            name="support-daytime-condition-1",
            position=(39, 10),
            sheet=sprite_sheet,
            spec=sprite_spec,
        )
        self.support_daytime_image_2 = SpriteImage(
            name="support-daytime-condition-2",
            position=(39, 10),
            sheet=sprite_sheet,
            spec=sprite_spec,
        )
        ...
        self.daytime_image_shadow = SpriteImage(
            name="daytime-condition-shadow",
        ...
        # moon phase actor
        self.moon_phase_image = SpriteImage(
            name="moon-phase", position=(39, 7), sheet=sprite_sheet, spec=sprite_spec
        )

        # timer actor
        self.timer_line = Line(
            name="timer-line", start=(0, 31), end=(63, 31), color=(255, 255, 0, 128)
        )
    ...
    def update_weather_data(self):
        try:
            self.logger.debug("Fetching current weather data")
            ...
            conditions_and_forecast = get_conditions_and_forecast_by_lat_long(
            ...
            if conditions_and_forecast:
                ...
                self.feels_like_str = (
                    f"FL {round(self.conditions_and_forecast['current']['feels_like'])}º"
                )
                self.dewpoint_str = (
                    f"Dew {round(self.conditions_and_forecast['current']['dew_point'])}º"
                )
                self.humidity_str = (
                    f"RH {round(self.conditions_and_forecast['current']['humidity'])}%"
                )
                self.low_temp_str = (
                    f"Low {round(self.conditions_and_forecast['daily'][0]['temp']['min'])}º"
                )
                self.high_temp_str = (
                    f"Hi {round(self.conditions_and_forecast['daily'][0]['temp']['max'])}º"
                )
                ...
                self.condition_short_desc = self.conditions_and_forecast["current"]["weather"][0][
                    "main"
                ]
                self.condition_long_desc = self.conditions_and_forecast["current"]["weather"][0][
                    "description"
                ]
                forecast_date_time = datetime.fromtimestamp(
                    self.conditions_and_forecast["daily"][0]["dt"]
                )
                self.is_daytime = None
                self.condition_short_desc_list = [
                    item["main"] for item in self.conditions_and_forecast["current"]["weather"]
                ]
                self.condition_long_desc_list = [
                    item["description"]
                    for item in self.conditions_and_forecast["current"]["weather"]
                ]
    ...
    def update_view(self, elapsed_time: float):
        time_since_last_update = (
            time.time() - self.last_weather_data_at
        ...
        if self.need_to_update_condition_desc_animation:
            ...
            if text_width > stage_width:
                ...
                pause_duration = 5.0
                self.logger.debug(
                    f"Adding scroll animation for condition text. "
                    f"Text width: {text_width} px, "
                    f"scroll distance: {scroll_distance}"
                )

                pause_1 = Still(
                    name="Condition-pause-1",
                    actor=self.condition_description_label,
                    duration=pause_duration,
                )

                scroll_left = StraightMove(
                    name="Condition-scroll-left",
                    actor=self.condition_description_label,
                    duration=scroll_duration,
                    distance=(-scroll_distance, 0),
                    easing=Easing.LINEAR,
                )

                pause_2 = Still(
                    name="Condition-pause-2",
                    actor=self.condition_description_label,
                    duration=pause_duration,
                )

                scroll_right = StraightMove(
                    name="Condition-scroll-right",
                    actor=self.condition_description_label,
                    duration=scroll_duration,
                    distance=(scroll_distance, 0),
                    easing=Easing.LINEAR,
                ...
                condition_sequence = Sequence(
                    name="Condition-scroll-sequence",
                    actor=self.condition_description_label,
                ...
                self.logger.debug(
                    f"Not adding scroll animation for condition text. Text width: {text_width} px"
                )
        ...
        if self.is_daytime != last_is_daytime:
            ...
            if self.fresh_weather_data:
                self.logger.debug(
                    f"Sunrise: {sunrise_str}, sunset: {sunset_str}, time of day: {tod_str}"
                )
            if self.fresh_weather_data:
        ...
        if self.is_moon_out != last_is_moon_out:
            tod_str = self.format_epoch_time(time_of_day)
            self.logger.debug(
                f"Moonrise: {self.format_epoch_time(self.moonrise)}, "
        ...
        if not self.is_daytime:
            ...
            if self.fresh_weather_data:
                self.logger.debug("Not showing moon phase")
        ...
        if self.is_daytime:
            if (
                200 <= self.condition_code <= 299
                or 600 <= self.condition_code <= 699
                or 300 <= self.condition_code <= 399
                or 500 <= self.condition_code <= 599
                or 500 <= self.condition_code <= 599
            ):
        ...
        if self.last_weather_data_at:
            ...
            relative_length = round(
                max(round(self.refresh_time - time_since_last_update), 0) * 64.0 / self.refresh_time
            )
        else:
            relative_length = 0
    ...
    def get_app_instance():
        ...
        env_refresh_time = int(
            app_runner.get_env_parameter(
                "REFRESH_TIME", "settings", "refresh_time", default=60 * 15
            )
