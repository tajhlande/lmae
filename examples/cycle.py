...
def run_apps_in_subprocess_cycle(app_script_list: list[str], cycle_timeout=600):
    ...
    while True:
        for app_script in app_script_list:
            ...
            asyncio.run(
                run_app_as_subprocess_with_timeout(
                    python_path, app_script_path, timeout=cycle_timeout
                )
            )
...
def load_app_instance(module_path: str, module_name: str, app_class_name: str) -> App:
    if module_name in sys.modules:
        ...
        try:
            ...
            raise RuntimeError(
                f"Unable to load spec for module {module_name} at path {module_path}"
            ) from None
        ...
        try:
            ...
            raise RuntimeError(
                f"Unable to load module {module_name} at path {module_path}"
            ) from None
    ...
    try:
        ...
        raise RuntimeError(
            f"Unable to instantiate app class {app_class_name} in module {module_name}"
        ) from None
...
def create_apps_list_from_app_names():
    ...
    return map(
        lambda config: load_app_instance(
            os.path.join(module_base_path, config["module_path"]),
            config["module_name"],
            config["app_class"],
        ),
        app_config_list,
